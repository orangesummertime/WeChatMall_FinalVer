        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('shangpinPandian','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('shangpinPandian','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('shangpinPandian','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('shangpinPandian','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/lingshipifajiaoyi/upload/shangpinPandianMuBan.xls"
                        >批量导入商品盘点数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('shangpinPandian','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="shangpinPandianUploadSuccess"
                                :on-error="shangpinPandianUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('shangpinPandian','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入商品盘点数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('shangpinPandian','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "shangpinPandian.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="shangpinPandianQianNumber"
                                      header-align="center"
                                      label="盘点前数量">
                        <template slot-scope="scope">
                            {{scope.row.shangpinPandianQianNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="shangpinPandianHouNumber"
                                      header-align="center"
                                      label="盘点数量">
                        <template slot-scope="scope">
                            {{scope.row.shangpinPandianHouNumber}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shangpinPandianChaNumber"
                                   header-align="center"
                                   label="差数量">
                        <template slot-scope="scope">
                            {{scope.row.shangpinPandianChaNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('shangpinPandian','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('shangpinPandian','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('shangpinPandian','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                ...
                shangpinId : null,
                yuangongId : null,
                shangpinPandianQianNumber : null,
                shangpinPandianHouNumber : null,
                shangpinPandianChaNumber : null,
                shangpinPandianContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '电子邮箱': 'yuangongEmail',
                //本表字段
                     '盘点前数量': "shangpinPandianQianNumber",
                     '盘点数量': "shangpinPandianHouNumber",
                     '差数量': "shangpinPandianChaNumber",
                     '盘点时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'shangpin_pandian',//当前表表名,
                        sumColum : 'shangpin_pandian_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'shangpin_pandian_name',//分组字符串字段
                        // types : 'shangpin_pandian_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "shangpin_pandian_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['shangpinPandianDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            shangpinPandianUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "shangpinPandian/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            shangpinPandianUploadError(data){
