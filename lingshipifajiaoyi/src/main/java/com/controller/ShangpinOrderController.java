    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(shangpinOrder !=null){
            ...
            //级联表 员工
            //级联表
            YuangongEntity yuangong = yuangongService.selectById(shangpinOrder.getYuangongId());
            if(yuangong != null){
...
, "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            if(address != null){
            BeanUtils.copyProperties( address , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "yuangongId"
, "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            if(shangpin != null){
            BeanUtils.copyProperties( shangpin , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "yuangongId"
, "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
            ...
            if(yonghu != null){
            BeanUtils.copyProperties( yonghu , view ,new String[]{ "id", "createTime", "insertTime", "updateTime", "yuangongId"
, "yonghuId"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
    ...
    */
    ...
    public R save(@RequestBody ShangpinOrderEntity shangpinOrder, HttpServletRequest request){
        ...
        if(false)
            ...
            shangpinOrder.setYuangongId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        else if("用户".equals(role))
...
//                            shangpinOrderEntity.setYuangongId(Integer.valueOf(data.get(0)));   //员工 要改的
...
//                            shangpinOrderEntity.setShangpinOrderCourierName(data.get(0));                    //派送人姓名 要改的
//                            shangpinOrderEntity.setShangpinOrderCourierNumber(data.get(0));                    //派送人联系方式 要改的
...
//            Integer yuangongId = Integer.valueOf(String.valueOf(params.get("yuangongId")));//员工
    ...
    */
    ...
    public R deliver(Integer id , HttpServletRequest request){
        ...
        String role = String.valueOf(request.getSession().getAttribute("role"));
        if(!"员工".equals(role)){
            return R.error("只有员工才能发货");
        }
        YuangongEntity yuangong = yuangongService.selectById(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        if(yuangong == null){
            return R.error("查不到员工");
        }
        ShangpinOrderEntity  shangpinOrderEntity = shangpinOrderService.selectById(id);
        shangpinOrderEntity.setYuangongId(yuangong.getId());
        shangpinOrderEntity.setShangpinOrderCourierName(yuangong.getYuangongName());
        shangpinOrderEntity.setShangpinOrderCourierNumber(yuangong.getYuangongPhone());
