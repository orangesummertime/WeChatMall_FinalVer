<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <view v-if="isAuth('shangpinPandian','新增')" @click="onAddTap()"
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.shangpinPandianName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('shangpinPandian','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                 <view v-show="item.shangpinPandianQianNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">盘点前数量</text>
                     <text style="width: 70%">{{item.shangpinPandianQianNumber}}</text>
                 </view>
                 <view v-show="item.shangpinPandianHouNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">盘点数量</text>
                     <text style="width: 70%">{{item.shangpinPandianHouNumber}}</text>
                 </view>
                 <view v-show="item.shangpinPandianChaNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">差数量</text>
                     <text style="width: 70%">{{item.shangpinPandianChaNumber}}</text>
                 </view>
                <view v-show="item.shangpinPandianContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">盘点备注</text>
                    <view style="width: 70%" v-html="item.shangpinPandianContent"></view>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('shangpinPandian','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
            </view>
        </view>
    </view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.page(`shangpinPandian`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('shangpinPandian', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
                ...
                let res = await this.$api.page(`shangpinPandian`, searchForm);
                res.data.list.forEach(function(item,index){
                    if(item.shangpinPandianContent!=null){
